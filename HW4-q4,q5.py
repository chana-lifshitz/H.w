...
def print_mermaid(tree):
    ...
    def helper(node, node_id):
        for child, direction in [(node.left, "l"), (node.right, "r")]:
            child_id = node_id + direction
            if child:
                print(f"{node_id} --> {child_id}(({child.value}))")
                helper(child, child_id)
            else:
                print(f"{node_id} ~~~ {child_id}(( ))")
                print(f"style {child_id} fill:#fff,stroke-width:0px")

    print(f"t(({tree.root.value}))")
    helper(tree.root, "t")
